 ...
 * <p>
 * This implementation will return the same {@link Producer} instance (if
 * transactions are not enabled) for the provided {@link Map} {@code configs}
 * and optional {@link Serializer} implementations on each
 * {@link #createProducer()} invocation.
 * <p>
 * If you are using {@link Serializer}s that have no-arg constructors and
 * require no setup, then simplest to specify {@link Serializer} classes against
 * {@link ProducerConfig#KEY_SERIALIZER_CLASS_CONFIG} and
 * {@link ProducerConfig#VALUE_SERIALIZER_CLASS_CONFIG} keys in the
 * {@code configs} passed to the {@link DefaultKafkaProducerFactory}
 * constructor.
 * <p>
 * If that is not possible, but you are sure that at least one of the following
 * is true:
 * <ul>
 * <li>only one {@link Producer} will use the {@link Serializer}s</li>
 * <li>you are using {@link Serializer}s that may be shared between
 * {@link Producer} instances (and specifically that their close() method is a
 * no-op)</li>
 * <li>you are certain that there is no risk of any single {@link Producer}
 * being closed while other {@link Producer} instances with the same
 * {@link Serializer}s are in use</li>
 * </ul>
 * then you can pass in {@link Serializer} instances for one or both of the key
 * and value serializers.
 * <p>
 * If none of the above is true then you may provide a {@link Supplier} function
 * for one or both {@link Serializer}s which will be used to obtain
 * {@link Serializer}(s) each time a {@link Producer} is created by the factory.
 * <p>
 * The {@link Producer} is wrapped and the underlying {@link KafkaProducer}
 * instance is not actually closed when {@link Producer#close()} is invoked. The
 * {@link KafkaProducer} is physically closed when
 * {@link DisposableBean#destroy()} is invoked or when the application context
 * publishes a {@link ContextStoppedEvent}. You can also invoke
 * {@link #reset()}.
 * <p>
 * Setting {@link #setTransactionIdPrefix(String)} enables transactions; in
 * which case, a cache of producers is maintained; closing a producer returns it
 * to the cache. The producers are closed and the cache is cleared when the
 * factory is destroyed, the application context stopped, or the
...
public class DefaultKafkaProducerFactory<K, V> implements ProducerFactory<K, V>, ApplicationContextAware, ApplicationListener<ContextStoppedEvent>, DisposableBean {
	 ...
	 * {@link ProducerConfig#TRANSACTIONAL_ID_CONFIG} if provided. This config is
	 * going to be overridden with a suffix for target {@link Producer} instance.
	 *
	 * @param configs         the configuration.
	 ...
	 * @param valueSerializerSupplier the value {@link Serializer} supplier
	 *                                function.
	 * @since 2.3
	 */
	...
	/**
	 * The time to wait when physically closing the producer via the factory rather
	 * than closing the producer itself (when {@link #reset()}, {@link #destroy()
	 * #closeProducerFor(String)}, or {@link #closeThreadBoundProducer()} are
	 * invoked). Specified in seconds; default
	 ...
	 * By default a {@link ProducerConfig#TRANSACTIONAL_ID_CONFIG} value from
	 * configs is used as a prefix in the target producer configs.
	 *
	...
	/**
	 * Set to true to create a producer per thread instead of singleton that is
	 * shared by all clients. Clients <b>must</b> call
	 * {@link #closeThreadBoundProducer()} to physically close the producer when it
	 * is no longer needed. These producers will not be closed by {@link #destroy()}
	 * or {@link #reset()}.
	 *
	...
	private void enableIdempotentBehaviour() {
		...
		if (Boolean.FALSE.equals(previousValue)) {
			LOGGER.debug(() -> "The '" + ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG + "' is set to false, may result in duplicate messages");
		}
	}
	 ...
	 *
	 * @return always true.
	 * @deprecated {@link org.springframework.context.Lifecycle} is no longer
	 *             implemented.
	 */
	@Deprecated
	...
	protected Producer<K, V> createKafkaProducer() {
		if (this.clientIdPrefix == null) {
			...
			newConfigs.put(ProducerConfig.CLIENT_ID_CONFIG, this.clientIdPrefix + "-" + this.clientIdCounter.incrementAndGet());
		...
		if (suffix == null) {
			...
			synchronized (this.consumerProducers) {
				if (!this.consumerProducers.containsKey(suffix)) {
					CloseSafeProducer<K, V> newProducer = doCreateTxProducer(txIdPrefix, suffix, this::removeConsumerProducer);
	...
	/**
	 * Subclasses must return a producer from the {@link #getCache()} or a new raw
	 * producer wrapped in a {@link CloseSafeProducer}.
	 *
	...
	private CloseSafeProducer<K, V> doCreateTxProducer(String prefix, String suffix, @Nullable Consumer<CloseSafeProducer<K, V>> remover) {
		...
		if (this.clientIdPrefix != null) {
			newProducerConfigs.put(ProducerConfig.CLIENT_ID_CONFIG, this.clientIdPrefix + "-" + this.clientIdCounter.incrementAndGet());
		}
		newProducer = createRawProducer(newProducerConfigs);
		newProducer.initTransactions();
		return new CloseSafeProducer<>(newProducer, getCache(prefix), remover, (String) newProducerConfigs.get(ProducerConfig.TRANSACTIONAL_ID_CONFIG));
	}
	...
	/**
	 * When using {@link #setProducerPerThread(boolean)} (true), call this method to
	 * close and release this thread's producer. Thread bound producers are
	 * <b>not</b> closed by {@link #destroy()} or {@link #reset()} methods.
	 *
	 * @since 2.3
	...
	protected static class CloseSafeProducer<K, V> implements Producer<K, V> {
		...
		public void commitTransaction() throws ProducerFencedException {
			...
			try {
				// intentional exception for example
		...
		public void close(@Nullable Duration timeout) {
			...
			if (this.cache != null) {
				Duration closeTimeout = this.txFailed instanceof TimeoutException ? CLOSE_TIMEOUT_AFTER_TX_TIMEOUT : timeout;
				if (this.txFailed != null) {
